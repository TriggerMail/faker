...
func (p Phone) phonenumber() string {
	...
	str := strings.Join(faker.IntToString(randInt), "")
...
func (p Phone) tollfreephonenumber() string {
	...
	for index, v := range faker.IntToString(ints) {
		if index == 3 {
			out += "-"
		}
		out += v
...
func (p Phone) e164PhoneNumber() string {
	...
	for _, v := range faker.IntToString(ints) {
		out += v
	}
	return fmt.Sprintf("+%s%s", boxDigitsStart[rand.Intn(1)], strings.Join(faker.IntToString(ints), ""))
}
